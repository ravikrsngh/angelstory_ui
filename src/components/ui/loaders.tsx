...
export const Loader = ({ label, isLoading, position }: LoaderProp) => {
  return isLoading ? (
    <div
      className={cn(
        "w-full h-full overflow-hidden flex items-center justify-center top-0 left-0 bg-black/70 z-[99999]",
        position
      )}
    >
      <button
        disabled
        type="button"
        className="text-white focus:ring-4 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 inline-flex items-center"
      >
        <svg
          aria-hidden="true"
          role="status"
